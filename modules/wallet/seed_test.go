...
func TestSweepSeedFunds(t *testing.T) {
	...
	_, err = wt.wallet.SendSiafunds(types.NewCurrency64(12), sk.UnlockHash())
	if err != nil {
		t.Fatal(err)
	}
	// Send some siacoins to the address, but not enough to cover the
	// transaction fee.
	_, err = wt.wallet.SendSiacoins(types.NewCurrency64(1), sk.UnlockHash())
	if err != nil {
		t.Fatal(err)
	}
...
func TestSweepSeedCoinsAndFunds(t *testing.T) {
	...
	// Send some siafunds to the address.
	for i := 0; i < 12; i++ {
		_, err = wt.wallet.SendSiafunds(types.NewCurrency64(1), sk.UnlockHash())
		if err != nil {
			t.Fatal(err)
		}
		if err := wt.addBlockNoPayout(); err != nil {
	...
	// fee.
	for i := 0; i < 100; i++ {
		_, err = wt.wallet.SendSiacoins(types.SiacoinPrecision.Mul64(10), sk.UnlockHash())
		if err != nil {
			t.Fatal(err)
		}
		if err := wt.addBlockNoPayout(); err != nil {
